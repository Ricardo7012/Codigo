...
SELECT N'Displaying non-default Sql configuration (sp_configure) values:' AS [Configuration];

SELECT N'Non-default values are just for information and are for troubleshooting reference.' AS [Trouble Shooting];

SELECT sc.NAME AS [Name]
	, sc.value_in_use AS [Value in Use]
	, DF.Value AS [DefaultValue]
FROM @tvDefaultValues DF
INNER JOIN sys.configurations sc
	ON sc.NAME LIKE '%' + DF.ConfigurationOption + '%'
		AND DF.Value <> sc.value_in_use
