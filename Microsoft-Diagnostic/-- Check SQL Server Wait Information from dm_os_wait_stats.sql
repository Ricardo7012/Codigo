...
*********************************/
SELECT TOP 25 os.wait_type AS 'Wait Type'
	, SUM(os.wait_time_ms) OVER (PARTITION BY os.wait_type) AS 'Sum Wait Time (ms)'
	, CAST(100. * SUM(os.wait_time_ms) OVER (PARTITION BY os.wait_type) / (1. * SUM(os.wait_time_ms) OVER ()) AS NUMERIC(12, 1)) AS '% Wait Time'
	, SUM(os.waiting_tasks_count) OVER (PARTITION BY os.wait_type) AS 'Sum Waiting Tasks'
	, CASE 
...
ORDER BY 'Sum Wait Time (ms)' DESC;
GO
...
SELECT b.sample_time AS 'Second Sample Time'
	, datediff(ss, wb1.sample_time, b.sample_time) AS 'Sample Duration (Sec)'
	, wd1.wait_type AS 'Wait Type'
	, cast((wd2.wait_time_ms - wd1.wait_time_ms) / 1000. AS NUMERIC(12, 1)) AS 'Wait Time (Sec)'
