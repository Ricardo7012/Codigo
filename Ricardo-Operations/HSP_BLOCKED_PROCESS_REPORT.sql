...
/****** Object:  Job [HSP Blocked Process Report]    Script Date: 1/31/2018 11:44:22 AM ******/
EXEC msdb.dbo.sp_delete_job @job_id=N'6a9fce4a-d995-46b9-9212-9004c938592d', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 1/31/2018 11:44:22 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'HSP Blocked Process Report', 
		@enabled=1, 
...
@notify_level_email=3 -- Notify on completion', 
		...
		@notify_email_operator_name=N'Ricardo', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Blocked process report]    Script Date: 1/31/2018 11:44:22 AM ******/
...
UPDATE @FullBlocking
	SET BlockingProcessSessionID = case when charindex(''@Sessionid'',replace(BlockingProcessInputBuf,'' '',''''),0) != 0 
...
UPDATE #DistinctTemp
	SET blocking_sessionID = case when charindex(''@Sessionid'',replace(blocking_inputbuf,'' '',''''),0) != 0 
...
EXEC msdb.dbo.Sp_send_dbmail
	...
	@subject = ''Daily Blocking Report'',
	@query = @query,
	@query_result_separator=''	'', 
	@query_result_width=32767,
	@query_attachment_filename = ''DailyBlockingResults.csv'',
...
Skip_Email:
', 
		@database_name=N'HSP_Prime', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'BlockedProcessReport', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
