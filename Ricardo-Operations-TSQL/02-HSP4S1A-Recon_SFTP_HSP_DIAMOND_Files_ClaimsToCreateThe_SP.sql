...
,ic.InterchangeName
into #EDI_HSPFiles
from HSP_RPT.dbo.EDIFILES edi with(nolock)   
left join HSP_RPT.[dbo].[InterchangeInfo] ic with(nolock) on ic.InterchangeId = edi.InterchangeID
where 1=1
...
into #ClaimInHSP
from #EDI_HSPFiles f
join HSP_RPT.dbo.EDIXacts l with(nolock) on l.EDIFileID = f.EDIFileID
join HSP_RPT.dbo.Claims c with(nolock) on c.ClaimId = l.ClaimID
...
CREATE CLUSTERED INDEX [IDX_TEMP_SFTP_ClaimsFoundInCM]
ON [dbo].[#SFTP_ClaimsFoundInCM] ([FileName],[DCN])
/**********************RICARDO*************************************************************************/
...
truncate table [HSP_Supplemental].[dbo].[EDI_SummaryRecon]
-- select * from [HSP_Supplemental].[dbo].[EDI_SummaryRecon]
insert into [HSP_Supplemental].[dbo].[EDI_SummaryRecon]
  ([Description]
      ,[FileCount]
      ,[ClaimCount])
