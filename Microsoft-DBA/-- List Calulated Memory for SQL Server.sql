-- List Calulated Memory for SQL Server
...
ELSE IF CAST(LEFT(CAST(SERVERPROPERTY('ResourceVersion') AS VARCHAR(20)), 2) AS INT) >= 11
...
-- Output findings
SELECT @CurrentMem AS 'Current SQL Memory Configuration'
	, @memInMachine AS 'Memory on Server'
	, @memOsTot AS 'Memory for OS'
	, @memForSql AS 'Memory for Sql'
	, 'EXEC sp_configure ''Max Server Memory'', ' + CAST(CAST(@memForSql * 1024 AS INT) AS VARCHAR(10)) + ' RECONFIGURE' AS 'Cmd to Execute'
