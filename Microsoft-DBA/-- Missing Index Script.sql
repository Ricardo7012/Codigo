...
SELECT TOP 25 dm_mid.database_id AS 'Database ID'
	, dm_migs.avg_user_impact * (dm_migs.user_seeks + dm_migs.user_scans) as 'Avg Estimated Impact'
	...
	, 'CREATE INDEX [IX_' + OBJECT_NAME(dm_mid.OBJECT_ID, dm_mid.database_id) + '_' + REPLACE(REPLACE(REPLACE(ISNULL(dm_mid.equality_columns, ''), ', ', '_'), '[', ''), ']', '') + CASE 
		...
		WHEN dm_mid.equality_columns IS NOT NULL
			...
			THEN ','
		ELSE ''
		END + ISNULL(dm_mid.inequality_columns, '') + ')' + ISNULL(' INCLUDE (' + dm_mid.included_columns + ')', '') AS 'Statement'
