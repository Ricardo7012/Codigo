-- List Index for Rebuild or Reorganize
...
BEGIN
	...
	-- If given database does not exist, raise error with 
	-- severity 20 in order to terminate script's execution
	...
	BEGIN
		...
		IF @checkResult < 1
			RAISERROR ('Error executing index reorganization/rebuild script: Database does not exist', 20, 1)
			WITH LOG
		...
		IF @checkResult < 90
			RAISERROR ('Error executing index reorganization/rebuild script: Only databases with SQL Server 2005 or later compatibility level are supported', 20, 1)
			WITH LOG
	END
	...
	-- If Always On Availability Groups are enabled, check for 
	-- primary databases (thus exclude secondary databases)
	IF @isHadrEnabled = 1
	BEGIN
	...
	--
	-- For each database included in the cursor, gather all 
	-- tables that have indexes with average fragmentation 
	-- percentage equal or above @fragmentationThreshold
	--
	OPEN dbNames_cursor
 ...
 INSERT INTO #tmpFragmentedIndexes (
  ...
  WHERE pst.index_id != 0  
   ...
   AND pst.avg_fragmentation_in_percent >= ' + @fragmentationThreshold + ''
	...
	END
	ELSE
		-- If 'report only' mode is disabled, then 
		-- execute index reorganize/rebuild statements
	BEGIN
