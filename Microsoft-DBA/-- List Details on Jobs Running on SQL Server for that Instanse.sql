-- List Details on Jobs Running on SQL Server for that Instance
...
--								  more friendly way of representing in Time format(HH:MM:SS)
...
--			6. Sampling			- Sampling is the number of history samples upon the calculation has been
--								  carried out.
...
--			6. Last_RunDuration	- shows the Duration of the last execution. This has been provided in a
--							  more friendly way of representing in Time format(HH:MM:SS)
...
--			1. Job ID			- Id of the Job.
--			2. Job Name		- Name of the Job.
--			3. Message		- If job has succeeded, then it will give the message. If the job has failed,
--							  then the message will have detailed error message.
...
--------------------------------------------------------------------------------------------
-- (Set @job_id = Null) to list all jobs running on SQL Server. If you want to
-- (Set @job_id = '1D239B27-EE12-4708-9091-C7AB717ADAAF') Drill into details on the job
--------------------------------------------------------------------------------------------

-- SET @job_id = Null
SET @job_id = '934B81E5-B514-48FB-9955-75D4F6B8F65F'
...
--As per few suggestions from my collegues, I added History details for a job if the job id is paased.
