-- Check the State of SQL Buffer Cache
...
SELECT dbo.udf_FormatNumber((a.cntr_value * 1.0 / b.cntr_value) * 100.0, 'kb') AS 'Buffer Cache Hit Ratio'
	, c.PageReadPerSec AS 'Page Reads (Sec)'
	, d.PageWritesPerSecond AS 'Page Writes (Sec)'
	, e.cntr_value AS 'User Connections'
	, f.cntr_value AS 'Page Life Expectency (Sec)'
	, g.CheckpointPagesPerSecond AS 'Check Points Pages (Sec)'
	, h.LazyWritesPerSecond AS 'Lazy Writes (Sec)'
	, dbo.udf_FormatNumber(i.cntr_value, 'kb') AS 'Free Space TempDB'
	...
	, dbo.udf_FormatNumber(m.cntr_value, 'kb') AS 'Target Memory'
	, dbo.udf_FormatNumber(n.cntr_value, 'kb') AS 'Total Memory'
	...
	, p.PercentSignalWait AS '% Signal Wait'
