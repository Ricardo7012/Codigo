...
$PodName = "ayun-sql03-activedr"                        # Pod name on the DR FlashArray
$SourceSQLServer = "ayun-sql19-03"                      # Production SQL Server

# Connect to Production VM
Write-Host "Connecting to Production VM..." -ForegroundColor Red
$SourceSQLServerSession = New-PSSession -ComputerName $SourceSQLServer 
...
Write-Host "Offlining the Production database(s)..." -ForegroundColor Red
$Query = "ALTER DATABASE ActiveDR_Example_DB SET OFFLINE WITH ROLLBACK IMMEDIATE"
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock {Param($querytask) Invoke-SQLcmd -ServerInstance . -Database master -Query $querytask} -ArgumentList ($Query)

# Offline the volume
Write-Host "Offlining the Production volume(s)..." -ForegroundColor Red
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq 'AC5FC11F8B3B49A00178C88B' } | Set-Disk -IsOffline $True }
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq 'AC5FC11F8B3B49A00178C88C' } | Set-Disk -IsOffline $True }

# Confirm state of disks
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Format-Table }
Break   # Safety-net to prevent accidental "run everything" of this script
...
$TargetSQLServer = "ayun-sql19-04"                      # DR SQL Server
...
$DiskOne = Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835C' }  }
$DiskTwo = Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835B' }  }
while (($DiskOne -eq $null) -or ($DiskTwo -eq $null)) {
    Write-Host "Waiting for DR volumes to come online" -ForegroundColor Red
    Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Update-HostStorageCache }
    $DiskOne = Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835C' }  }
    $DiskTwo = Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835B' }  }
}
...
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835C' } | Set-Disk -IsOffline $False }
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835B' } | Set-Disk -IsOffline $False }

# Setting volumes to Read/Write
# NOTE: Change disk serial number(s) to appropriate disks
Write-Host "Setting Read/Write for DR volumes..." -ForegroundColor Red
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835C' } | Set-Disk -IsReadOnly $False }
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835B' } | Set-Disk -IsReadOnly $False }
...
# Online the database
Write-Host "Onlining database ActiveDR_Example_DB in DR..." -ForegroundColor Red
$Query = "ALTER DATABASE ActiveDR_Example_DB SET ONLINE WITH ROLLBACK IMMEDIATE;"
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock {Param($querytask) Invoke-SQLcmd -ServerInstance . -Database master -Query $querytask} -ArgumentList ($Query)
...
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835C' } | Set-Disk -IsOffline $True }
Invoke-Command -Session $TargetSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq '81F096D1C1642A69017A835B' } | Set-Disk -IsOffline $True }
...
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq 'AC5FC11F8B3B49A00178C88B' } | Set-Disk -IsOffline $False }
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq 'AC5FC11F8B3B49A00178C88C' } | Set-Disk -IsOffline $False }

# Setting volumes to Read/Write
# NOTE: Change disk serial number(s) to appropriate disks
Write-Host "Setting Read/Write for Production volumes..." -ForegroundColor Red
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq 'AC5FC11F8B3B49A00178C88B' } | Set-Disk -IsReadOnly $False }
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock { Get-Disk | Where-Object { $_.SerialNumber -eq 'AC5FC11F8B3B49A00178C88C' } | Set-Disk -IsReadOnly $False }
...
# Online the database
Write-Host "Onlining database ActiveDR_Example_DB in Production..." -ForegroundColor Red
$Query = "ALTER DATABASE ActiveDR_Example_DB SET ONLINE WITH ROLLBACK IMMEDIATE;"
Invoke-Command -Session $SourceSQLServerSession -ScriptBlock {Param($querytask) Invoke-SQLcmd -ServerInstance . -Database master -Query $querytask} -ArgumentList ($Query)
